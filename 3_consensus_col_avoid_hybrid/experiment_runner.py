...
def _common_cli(ns: argparse.Namespace) -> list[str]:
    ...
    return [
        ...
        "--safety-enabled", _bool_str(cfg.hybrid_enabled),
        "--safety-method", "hybrid",
...
def main() -> None:
    ...
    try:
        print(f"[Runner] Starting experiment with n_agents={cfg.n_agents}, model={cfg.model}, outer_steps={cfg.outer_steps}, use_lexicographic={cfg.use_lexicographic}, hybrid_enabled={cfg.hybrid_enabled}.")
        procs.append(subprocess.Popen([py, str(here / 'logger_node.py'), *common], cwd=here))
