...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--traj-csv", type=str, default="consensus_traj.csv")
...
def _load_metrics(path: Path):
    ...
    if data.ndim != 2 or data.shape[1] < 14:
        raise ValueError("Unexpected metrics CSV format.")
    out = {}
    has_pair_num = data.shape[1] >= 15
    has_circle_num = data.shape[1] >= 16
    for row in data:
        k = int(row[0])
        out[k] = {
            "Vr": float(row[3]),
...
def main() -> None:
    ...
    def _apply_frame(ti: int) -> None:
        ...
        kk = int(k_unique[ti])
        ax.set_title(
            f"Inner step k={kk}/{k_max} | {'double' if is_double else 'single'} integrator"
        )
