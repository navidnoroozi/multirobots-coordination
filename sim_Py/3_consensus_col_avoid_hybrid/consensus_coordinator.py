...
def main() -> None:
    ...
    while True:
        ...
        if not all(bool(replies[i].get("ok", False)) for i in range(1, cfg.n_agents + 1)):
            ...
            rep.send(dumps(make_envelope("coord_reply", {"ok": False, "outer_index": outer_j, "neighbors": nbrs, "model": cfg.model, "M": M, "U_seq": U.tolist(), "replies": replies, "note": "At least one controller failed; returning zeros."}, src="coord")))
            continue

        U = np.zeros((M, cfg.n_agents, cfg.dim), dtype=float)
        diag: Dict[str, Dict[int, Any]] = {k: {} for k in ["objective_primary", "lex_used", "phi_terminal", "ri_terminal", "r_term", "diam_C", "t_primary_ms", "t_lex_ms", "t_total_ms", "n_var_primary", "n_eq_primary", "n_ineq_primary", "n_var_lex", "n_eq_lex", "n_ineq_lex"]}
        for i in range(1, cfg.n_agents + 1):
            rep_i = replies[i]
            u_seq = np.array(rep_i["u_seq"], dtype=float)
            U[:, i - 1, :] = u_seq
            for k in diag.keys():
                diag[k][i] = rep_i.get(k, np.nan if 'lex' not in k and 'var' not in k and 'eq' not in k and 'ineq' not in k else 0)
            u_prev[i] = u_seq[-1, :].copy()
        rep.send(dumps(make_envelope("coord_reply", {"ok": True, "outer_index": outer_j, "neighbors": nbrs, "U_seq": U.tolist(), "diag": diag}, src="coord")))
