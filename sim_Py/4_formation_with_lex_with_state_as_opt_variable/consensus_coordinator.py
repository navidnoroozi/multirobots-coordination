...
def main() -> None:
    ...
    for i in range(1, cfg.n_agents + 1):
        ...
        print(f"[Coordinator] REQ -> ctrl{i} at {ep}")
    ...
    while True:
        ...
        for i in range(1, cfg.n_agents + 1):
            ...
            U[:, i - 1, :] = u_seq
            for k in diag.keys():
                default = np.nan if ("lex" not in k and "var" not in k and "eq" not in k and "ineq" not in k) else 0
                diag[k][i] = rep_i.get(k, default)
