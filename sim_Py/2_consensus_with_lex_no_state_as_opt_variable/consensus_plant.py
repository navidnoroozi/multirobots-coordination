...
def _get_keyed(d: dict, i: int, default):
    ...
    Read d[i] if exists, else d[str(i)], else default.
    """
...
def main() -> None:
    cfg = NetConfig()
    M = cfg.horizon_M()
    print(f"[Plant] model={cfg.model} | auto_M={cfg.auto_M} | M={M} | outer_steps={cfg.outer_steps}")
    ...
    with traj_path.open("w", newline="") as f_traj, metrics_path.open("w", newline="") as f_met, outer_path.open("w", newline="") as f_out:
        ...
        for outer_j in range(cfg.outer_steps):
            # Request U sequence + diagnostics
            try:
                req.send(
                    dumps(
                        make_envelope(
            ...
            # Log outer-step diagnostics if available
            ...
            if reply.get("ok", False) and isinstance(diag, dict) and diag:
                ...
                # still write rows (NaNs) for alignment
                for i in range(1, cfg.n_agents + 1):
                    w_out.writerow([
                                    outer_j, i, cfg.model, M,
                                    np.nan, 0, np.nan, 0, np.nan, np.nan, np.nan,
