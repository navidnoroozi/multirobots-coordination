...
u_var = sprintf('u_ref_%d', agent_id); 
...
% 1. Constant Reference (Updated dynamically via set_param)
...
build_unicycle_dynamics(ud, sim_dt, agent_id);
...
% Wires (Replace 'FromWS/1' with 'Constant_u/1')
...
% ====================================================================
% ====================================================================
% Motion Controller (Input-Output Feedback Linearization)
% ====================================================================
function build_motion_ctrl(mc, k_heading, k_speed, max_v, max_omega)
    ...
    % In1 = u_ref (2x1), In2 = theta (1x1)
    in1 = [mc '/In1'];
    in2 = [mc '/In2'];
    add_block('built-in/Inport', in1, 'Position', [20 40 50 60]);
    add_block('built-in/Inport', in2, 'Position', [20 100 50 120], 'Port','2');

    % Demux u_ref into u_x and u_y
    dmx_u = [mc '/Demux_u'];
    add_block('simulink/Signal Routing/Demux', dmx_u, ...
        'Outputs', '2', 'Position', [70 30 75 80]);
    add_line(mc, 'In1/1', 'Demux_u/1');
    
    % Mux to combine [u_x, u_y, theta] into a single 3x1 vector u
    mux = [mc '/Mux_in'];
    add_block('simulink/Signal Routing/Mux', mux, ...
        'Inputs', '3', 'Position', [110 30 115 130]);
    add_line(mc, 'Demux_u/1', 'Mux_in/1'); % u_x -> u(1)
    add_line(mc, 'Demux_u/2', 'Mux_in/2'); % u_y -> u(2)
    add_line(mc, 'In2/1', 'Mux_in/3');     % theta -> u(3)

    % Linearization for v = u_x*cos(theta) + u_y*sin(theta)
    fcn_v = [mc '/Fcn_v'];
    ...
    % Linearization for omega = (1/L) * (-u_x*sin(theta) + u_y*cos(theta))
    % Using Look-ahead distance L = 0.15 meters (1 / 0.15 = 6.6667)
    fcn_w = [mc '/Fcn_w'];
    ...
    % Saturation for Angular Velocity (omega)
    sat_w = [mc '/Sat_w'];
...
% Unicycle Dynamics
% ====================================================================
function build_unicycle_dynamics(ud, sim_dt, agent_id)
